...
export const EditableInspectionHistoryView = () => {
  ...
  const {
    ...
    isLoading
  } = useInspectionRecords(selectedProperty?.id);
  const {
    data: templates = []
  ...
  Object.values(groupedRecords).forEach((outerGroup: any) => {
    if (outerGroup.templates) {
      ...
      Object.values(outerGroup.templates).forEach((templateGroup: any) => {
        templateGroup.records.sort((a: InspectionRecord, b: InspectionRecord) => new Date(b.inspection_date).getTime() - new Date(a.inspection_date).getTime());
      });
    } else {
      // Single Property mode: Template only
  ...
  const updateInspectionItem = (itemId: string, updates: Partial<InspectionItem>) => {
    ...
    const updatedItems = editedRecord.items.map(item => item.id === itemId ? {
      ...item,
      ...updates
    } : item);
    setEditedRecord({
  ...
  return <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant={showCustomReport ? "default" : "outline"} size="sm" onClick={() => setShowCustomReport(!showCustomReport)}>
                            ...
                            {group.records.map((record: InspectionRecord) => <div key={record.id} className="border rounded-lg">
                                <div className="p-1.5 cursor-pointer hover:bg-accent/20 transition-colors" onClick={() => toggleRecord(record.id)}>
                                   <div className="flex items-center justify-between">
                                    ...
                                    {editingRecordId !== record.id && <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                                        <Button size="sm" variant="ghost" onClick={() => startEditingRecord(record)}>
                                          <Edit className="h-4 w-4" />
                                        </Button>
                                        <Button size="sm" variant="ghost" onClick={() => deleteInspectionRecord(record.id)}>
                                ...
                                {expandedRecords[record.id] && <div className="border-t p-1.5 space-y-1 bg-muted/20">
                                    {editingRecordId === record.id ? <>
                                        {editedRecord?.items.map(item => <div key={item.id} className="flex items-start gap-2 p-1.5 bg-background rounded border">
                                            <Checkbox checked={item.completed} onCheckedChange={checked => updateInspectionItem(item.id, {
                            completed: checked as boolean
                          })} className="mt-0.5" />
                                            <div className="flex-1 min-w-0 space-y-1">
                                              <div className="text-xs font-medium">{item.description}</div>
                                              <Textarea value={item.notes || ''} onChange={e => updateInspectionItem(item.id, {
                              notes: e.target.value
                            })} placeholder="Notes..." className="text-xs min-h-[40px] max-h-[80px] overflow-y-auto" />
                                            </div>
                                          </div>)}
                                        ...
                                        {record.items.map(item => <div key={item.id} className="flex items-start gap-1.5 p-1 bg-background rounded text-xs">
                                            <Checkbox checked={item.completed} disabled className="mt-0.5 h-3 w-3" />
                                            <div className="flex-1 min-w-0 flex items-start gap-1.5">
                                              <span className="font-medium">{item.description}</span>
                                              {item.notes && <span className="text-muted-foreground">- <span className="inline-block max-h-[40px] overflow-y-auto align-top">{item.notes}</span></span>}
                                            </div>
                                          </div>)}
