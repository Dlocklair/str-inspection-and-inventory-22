...
export const InventorySection = () => {
  ...
  const {
    ...
    updateItem,
    addItem
  } = useInventoryItems(queriesEnabled);
  const {
    data: categories = []
  } = useInventoryCategories(queriesEnabled);
  ...
  const handleSendRestockEmails = async () => {
    const itemsNeedingRestock = items.filter(item => item.current_quantity <= item.restock_threshold);
    ...
    try {
      ...
      const {
        error
      } = await supabase.functions.invoke('send-inventory-emails', {
        body: {
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="inventory" className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <Input placeholder="Search inventory..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="max-w-sm" />
            ...
            {lowStockItems.length > 0 && <Button onClick={handleSendRestockEmails} variant="outline">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={newItem.category_id} onValueChange={value => setNewItem({
                  ...newItem,
                  category_id: value
                })}>
                      <SelectTrigger>
                ...
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="number" min="0" value={newItem.restockLevel} onChange={e => setNewItem({
                  ...newItem,
                  restockLevel: parseInt(e.target.value) || 5
                })} />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="number" min="0" step="0.01" value={newItem.cost} onChange={e => setNewItem({
                  ...newItem,
                ...
                <div className="space-y-2">
                  ...
                  <Input value={newItem.supplier} onChange={e => setNewItem({
                ...newItem,
          ...
          }} onSave={handleEditSave} onCancel={() => setEditingItem(null)} onDelete={() => setItemToDelete(editingItem.id)} categories={categories.map(c => c.name)} onAssignToProperty={() => setShowPropertyAssignment(true)} />
              
              {/* Property Assignment Dialog */}
              <InventoryPropertyAssignment item={editingItem} open={showPropertyAssignment} onOpenChange={setShowPropertyAssignment} onAssign={handleAssignToProperties} />
            </>}
